	...
	while (read_pos < ips_patch.size()) {
		...
		if (write_size > 0) {
			...
			if (read_pos + write_size > ips_patch.size()) {
				...
				error += std::to_string(read_pos);
				error += " length ";
				error += std::to_string(write_size);
				error += ".";
				throw MalformedDataException(error);
			}
		} else {
...
const std::vector<RomPatch> SRAM_PATCHES_FLASH1M_V102 = {
	RomPatch(
		{
...
const std::vector<RomPatch> SRAM_BANK_1_PATCHES_FLASH1M_V102 = {
	RomPatch(
		{
...
const std::vector<RomPatch> SRAM_PATCHES_FLASH1M_V103 = {
	RomPatch(
		{
			0x05, 0x4b, 0xaa, 0x21, 0x19, 0x70, 0x05, 0x4a, 0x55, 0x21, 0x11, 0x70, 0xb0, 0x21, 0x19, 0x70,
...
const std::vector<RomPatch> SRAM_BANK_1_PATCHES_FLASH1M_V103 = {
	RomPatch(
		{
			0x05, 0x4b, 0xaa, 0x21, 0x19, 0x70, 0x05, 0x4a, 0x55, 0x21, 0x11, 0x70, 0xb0, 0x21, 0x19, 0x70,
			0xe0, 0x21, 0x09, 0x05, 0x08, 0x70, 0x70, 0x47
...
const std::vector<RomPatch> SRAM_PATCHES_FLASH512_V13X = {
	RomPatch(
		{
...
const std::vector<RomPatch> SRAM_PATCHES_EEPROM_V12X = {
	RomPatch(
...
const std::vector<RomPatch> SRAM_PATCHES_EEPROM_V124 = {
...
const std::vector<RomPatch> SRAM_PATCHES_EEPROM_V126 = {
	RomPatch(
...
const std::vector<RomPatch> SRAM_PATCHES_FLASH_V12X = {
	RomPatch(
...
const std::vector<RomPatch> SRAM_PATCHES_FLASH_V12Y = {
	RomPatch(
...
const std::vector<RomPatch> SRAM_PATCHES_FLASH_V12Z = {
...
std::vector<ByteOffset> patch_eepromv111(std::vector<unsigned char> & rom_data, const bool interchangeable_empty_byte = true) {
	...
	const std::vector<unsigned char> find1 = {0x0e, 0x48, 0x39, 0x68, 0x01, 0x60, 0x0e, 0x48};
	const std::vector<unsigned char> replacement1 = {0x00, 0x48, 0x00, 0x47, 0, 0, 0, 0x08};
	const std::vector<unsigned char> find2 = {0x27, 0xe0, 0xd0, 0x20, 0x00, 0x05, 0x01, 0x88};
	const std::vector<unsigned char> replacement2 = {0x27, 0xe0, 0xe0, 0x20, 0x00, 0x05, 0x01, 0x88};

	const std::vector<unsigned char> footer = {
...
std::vector<ByteOffset> patch_sram_by_type(std::vector<unsigned char> & rom_data, const gba::SaveType save_type,
						const unsigned char sram_bank_type, const bool interchangeable_empty_byte) {
	std::vector<ByteOffset> results;
	...
	switch (save_type) {
		...
		case gba::FLASH1M_V102:
		    switch (sram_bank_type) {
		        case 1:
		            results = patch_sram_by_set(rom_data, SRAM_BANK_1_PATCHES_FLASH1M_V102);
			        break;
			    default:
		        case 0:
		...
		case gba::FLASH1M_V103:
		    switch (sram_bank_type) {
		        case 1:
		            results = patch_sram_by_set(rom_data, SRAM_BANK_1_PATCHES_FLASH1M_V103);
			        break;
			    default:
		        case 0:
...
std::vector<ByteOffset> patch_sram(std::vector<unsigned char> &rom_data, unsigned char sram_bank_type) {
	std::vector<ByteOffset> results;
	...
	for (const gba::SaveType & save_type : gba::SAVE_TYPES) {
		...
		if (pattern.size() > 0 && find_bytes(rom_data, pattern)) {
			std::vector<ByteOffset> foo = patch_sram_by_type(rom_data, save_type, sram_bank_type);
